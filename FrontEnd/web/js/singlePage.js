...
$(window).on("load", async function () {
    ...
    console.log("este es el id de la pelicula", window.idDePelicula)
    let informacionPeliculaIndividual


    try {
        let informacionDePelicula = await fetch(
                `https://data-imdb1.p.rapidapi.com/movie/id/${window.idDePelicula}/`,
                {
                    method: "GET",
                    headers: {
        ...
        console.log(informacionDePelicula.Data);

        // informacionFinalPeliculas.push(informacionDePelicula[idDePeliculas[x].title])
        informacionPeliculaIndividual = informacionDePelicula.Data;
        console.log("Esta es la informacion de la peli", informacionPeliculaIndividual)
    } catch (err) {

    }


    document.getElementById("imagen_movie_single").src = informacionPeliculaIndividual.image_url
    document.getElementById("titulo_pelicula").innerHTML = informacionPeliculaIndividual.title

    document.getElementById("link_pelicula").href = informacionPeliculaIndividual.trailer
    document.getElementById("trama_pelicula").innerHTML = informacionPeliculaIndividual.plot
    document.getElementById("resumen_pelicula").innerHTML = informacionPeliculaIndividual.description
    document.getElementById("ano_pelicula").innerHTML = informacionPeliculaIndividual.release
    document.getElementById("duracion_pelicula").innerHTML = `${informacionPeliculaIndividual.movie_length} min`
    document.getElementById("rating_pelicula").innerHTML = informacionPeliculaIndividual.content_rating
    document.getElementById("calificacion").innerHTML = `<span>${informacionPeliculaIndividual.rating}</span> /10`
    console.log("esta es el ratin", informacionPeliculaIndividual.rating)
    for (let x in informacionPeliculaIndividual.gen) {
        document.getElementById("generos_pelicula").innerHTML += `${informacionPeliculaIndividual.gen[x].genre}. `

    }

    for (let x in informacionPeliculaIndividual.keywords) {
        document.getElementById("terminos_busqueda").innerHTML += ` <span class="time"><a href="#">${informacionPeliculaIndividual.keywords[x].keyword}</a></span>`

    }

    let calificacion = document.getElementById("calificacionTES")

    console.log("calificacion", window.calificacionTESJS)

    
    for (let i = 0; i < window.calificacionTESJS; i++) {
        
        calificacion.innerHTML += `<i class="ion-ios-star"></i>`
    }
    
    let estrellasRestantes = 10 -window.calificacionTESJS
    
    console.log("restante", estrellasRestantes)
    
   
    
    for (let x = 0; x < estrellasRestantes; x++) {
        
        calificacion.innerHTML += `<i class="ion-ios-star-outline"></i>`
    }
